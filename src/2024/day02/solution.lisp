...
;; really appreciate your help analyzing some unusual data from the
;; Red-Nosed reactor. You turn to check if The Historians are waiting
;; for you, but they seem to have already divided into groups that are
;; currently searching every corner of the facility. You offer to help
...
(defun increasing-p (list)
  (and (> (length list) 1)              ; At least 2 elements
       (every #'(lambda (a b)
                  (and (< a b)          ; Must be strictly increasing
                       (valid-change-p a b)))
              list
              (cdr list))))

(defun decreasing-p (list)
  (and (> (length list) 1)  ; At least 2 elements
       (every #'(lambda (a b)
                  (and (> a b)  ; Must be strictly decreasing
                       (valid-change-p a b)))
              list
              (cdr list))))

(defun safe-report-p (report)
  (or (increasing-p report)
      (decreasing-p report)))

(defun count-safe-reports (reports)
  (count-if #'safe-report-p reports))

(defun solve-part1 (filename)
...
(defun safe-report-with-dampener-p (report)
  (or (safe-report-p report)  ; First check if it's already safe
      (loop for i from 0 below (length report)
            thereis (let ((modified-report (remove-nth i report)))
                     (safe-report-p modified-report)))))

(defun count-safe-reports-with-dampener (reports)
  (count-if #'safe-report-with-dampener-p reports))

(defun solve-part2 (filename)
  (let ((reports (read-reports filename)))
    (count-safe-reports-with-dampener reports)))

(defun test-input ()
  (aoc:input-file 2024 2 :test t))

(defun puzzle-input ()
  (aoc:input-file 2024 2))
