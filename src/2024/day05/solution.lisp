...
(defun before (x y seq &key (test #'eql))
  "Check if X appears before Y in SEQ."
  (let ((pos-x (position x seq :test test))
        (pos-y (position y seq :test test)))
    (and pos-x pos-y (< pos-x pos-y))))
...
(defun test-input ()
  (aoc:input-file 2024 5 :test t))

(defun puzzle-input ()
  (aoc:input-file 2024 5))
